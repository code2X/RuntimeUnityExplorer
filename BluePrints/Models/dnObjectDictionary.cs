...
{
    [Serializable]
    public class dnObjectDictionary<T> where T : dnObject
    {
        ...
        public int ObjectCount => m_ID2Objs.Count;

        public void Visit(Vistors.EventVistor<T> vistor)
        {
            foreach (var pair in m_ID2Objs)
            {
                bool onEvent = false;
                vistor?.Invoke(pair.Value, out onEvent);
                if (onEvent)
                    break;
            }
        }

        public void Visit(Vistors.Vistor<T> vistor)
        {
            foreach (var pair in m_ID2Objs)
            {
                vistor?.Invoke(pair.Value);
            }
        }
