...
namespace DotInsideNode
{
    ...
    {
        int AddNode(INode nodeView, bool mousePos = true);
        bool Contain(int nodeID);
        bool DeleteNode(int nodeID);
        void Clear();

        bool GetNodeEditorPostion(INode node,out Vector2 position);
        bool SetNodeEditorPostion(INode node, Vector2 position);
        Dictionary<int, Vector2> NodeEditorPostions
        {
            get;
            set;
        }
